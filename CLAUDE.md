...
`OPENAI_API_KEY` is optional but required by `/chatbot` (**Nacho**, the admin assistant): without it the endpoint answers 503 and the rest of the API keeps working. `OPENAI_MODEL` overrides the model (default `gpt-5.6-luna`); the chatbot uses the OpenAI **Responses** API with `store: false` + `include: ["reasoning.encrypted_content"]`, so no conversation is retained on OpenAI's side (the front resends the whole item history each turn). `POST /chatbot` answers JSON by default and **Server-Sent Events** when the client sends `Accept: text/event-stream` (events `turno`, `estado`, `herramienta`, `texto`, `archivo`, `final`, `error`; the front's `postStream` consumes them). `POST /chatbot` also accepts an optional `archivo` (the DDJJ Excel an admin was forwarded): it uses its own **memory-storage** multer, never the shared `multerconfig.js` — that one writes into `uploads/`, which is served statically, and a payroll file must not end up public. `utils/chatbotExcel.js` runs it through the same `validateEmployees` as the real import so the diagnosis matches what the company would see.

Nacho's usage is metered: `models/chatbotUsoModel.js` writes one row per query into `chatbot_uso` (tokens in/cached/out, turns, tools, duration, cost in USD) and keeps the single-row `chatbot_plan` (mode `prueba`/`plan`, monthly `consultas_incluidas`, `tope_usd`). DDL lives in `db/migrations/2026_09_chatbot_uso.sql` but the model also ensures the tables on first use, so a deploy without the migration still works. The billing unit is the **query**, not the token — one question can hit the model up to 8 times. Cost comes from `utils/chatbotCostos.js` with the rates in `OPENAI_PRECIO_ENTRADA` / `_ENTRADA_CACHEADA` / `_SALIDA` (USD per 1M tokens); every read recalculates from the stored tokens with today's rates, so a wrong or missing rate never corrupts history. `GET /chatbot/uso` returns the period quota the panel draws. The provider-side numbers live **outside** the client's panel: `routes/facturacionRoute.js` is mounted before the `authRequired` barrier and serves `GET /facturacion/uso`, `GET /facturacion/reporte` (cost, projection, suggested quota and price, margin against `?precio_usd=`) and `PUT /facturacion/plan` to whoever sends the `x-facturacion-token` header matching `CHATBOT_FACTURACION_TOKEN` — that is Lumarsoft's own site, not a user of the system. Without that variable the whole router answers 404. `CHATBOT_EMAILS_PROVEEDOR` can additionally whitelist panel admins to see the dollar figures inside the panel, and is empty by default, so no cost figure reaches a UESEVI admin's browser. `POST /chatbot` answers 429 when the monthly `tope_usd` is reached (any mode) or, with `modo = 'plan'`, when the included queries run out; `modo = 'prueba'` only measures. A query that errors or gets cancelled is still recorded: those tokens were already paid for.

Nacho's files: `utils/chatbotPrompt.js` (static instructions + per-request `contextoDelDia` with today's date and the period "en término", appended at the **end** of the input so the dynamic part never invalidates the cached prefix — moving it back to the front silently multiplies the token bill), `utils/chatbotTools.js` (tool definitions, UI labels, the in-memory write proposals), `utils/chatbotSql.js` (read-only SQL validator: single statement, table whitelist, no sensitive columns, and a row cap that understands the three MySQL `LIMIT` forms so paginating with `OFFSET` works), `models/chatbotModel.js` (every query mirrors the criteria of the admin screens: `JOIN_VIGENTE`, `auxiliar` breakdown via `MAX(id)`, `pagos_panel` grouped, `estado NULL` = legacy migrated rows never counted as pending, `empresas.estado` is `Activo`/`Inactivo`/`Pendiente`). **A zero balance is not the same as being up to date**: a company that never filed has nothing to owe *because there is no filing*, so `missingPeriods` anchors the range at `empresas.created` when there are no statements at all and flags `sin_declaraciones`; the tools turn that into an explicit warning, and the listing is capped (the true counts and the real first/last period travel in their own fields — never infer the range from the truncated list). `debtorCompanies` computes the mora per statement with `calcInterest` and returns `totales` across every debtor company, so the total debt comes from one place instead of the model adding up numbers it read elsewhere. The model never does arithmetic: debt with interest comes from `companyDebt`, which calls `utils/interest.js` like the Panel de Pagos. Writes are proposals confirmed by the admin through `/chatbot/confirm`. Nacho's queries need `pagos_panel` and the presentismo columns, so a local DB restored from the baseline dump must have `db/migrations/2026_07_pagos_panel.sql`, `2026_08_pagos_panel_aplica_interes.sql` and `2026_08_presentismo.sql` applied.
